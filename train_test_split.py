...
def my_train_test_split(participant=None):
    ...
    #removed 'max_pupil', 'avd_preceding_pupil', 'avg_pupil', 'change_pupil', 'rt', 'rtResidual', 'rtResidualC'
    df = df[['max(RIGHT_FIX_INDEX)', 'avg(RIGHT_IN_BLINK)', 'avg(RIGHT_IN_SACCADE)', 'avg(RIGHT_PUPIL_SIZE)', 'stimnum', 'acc', 'rt_rec', 'trialno_rec', 'ratingConfidence', 'pupil_deviation', 'rt_deviation', 'acc_rec']]
    ...
    for i in range(len(x1)):
        if len(x1[i]) == 11:  # CHANGE VALUE DEPENDING ON NUM OF COLUMNS IN X
            x.append(x1[i])
            y.append(y1[i])
    x = np.asarray(x)
    y = np.asarray(y)
